...
export class SpeechService implements OnDestroy {
    ...
    private curentWindow: any = window;

    constructor(
        private zone: NgZone,
        @Inject('SPEECH_LANG') public lang: string,
    ) {
        if (this.curentWindow['SpeechRecognition'] || this.curentWindow['webkitSpeechRecognition']) {
            this.init();
        }
    }

    private init() {
        const SpeechRecognition = this.curentWindow['speechRecognition'] || this.curentWindow['webkitSpeechRecognition'];
    ...
    public declareContext(context: string[]): void {
    ...
    public declareCommand(command: any, context: any): void {
        const contextKey = context.map((w: any) => w.toLowerCase()).join('/');
        if (!this.commands[contextKey]) {
            this.commands[contextKey] = {};
        }
    ...
    public getContextForWord(word: string): string | null {
    ...
    private setGrammar(): void {
        const SpeechGrammarList = this.curentWindow['speechGrammarList'] || this.curentWindow['webkitSpeechGrammarList'];
